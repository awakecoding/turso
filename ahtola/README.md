...
All managed assemblies target `net8.0`, `net9.0`, and `net10.0`. .NET Framework hosts (including Windows PowerShell 5.1 / `net48`) are unsupported: there are no `net48` assets and none are planned. File-backed databases additionally require Windows or 64-bit Linux at runtime; 32-bit Linux (`linux-x86`) is unsupported, as noted in the limitation list below.
...
`make restore`, `make build`, `make test`, and `make pack` use the managed-only release path. There is no Rust toolchain and no native companion. `Ahtola.slnx` contains only the managed package projects plus tests and benchmarks. `make test` validates the packed provider and EF Core facade by restoring, building, running, and publishing the source-free `ManagedPackageConsumer` sample. `make validate-managed-project-closure` rejects native package, asset, P/Invoke, and Rust-tool references from the managed package configurations. `scripts/Validate-ManagedPackageClosure.ps1` opens every managed `.nupkg`, rejects native entries and native/PInvoke/Rust build configuration, and checks the restored closure and publish output. `make validate-managed-package` runs the same packaged-provider validation directly.
...
Remote Hrana and embedded replicas reject `ATTACH` and `DETACH` before network
execution; syncing or routing an attached database is not implied. The SQLite
