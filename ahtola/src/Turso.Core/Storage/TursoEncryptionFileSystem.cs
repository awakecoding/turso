...
/// <summary>
/// Supplies Turso page-encryption options to a managed file database opened
...
/// </remarks>
public sealed class TursoEncryptionFileSystem : IFileSystem, IDisposable
{
    private readonly IFileSystem _inner;
    private readonly bool _ownsEncryption;
    private TursoEncryptionOptions? _encryption;
    ...
    /// </summary>
    public TursoEncryptionFileSystem(IFileSystem inner, TursoEncryptionOptions encryption)
    {
    ...
    private TursoEncryptionFileSystem(
        IFileSystem inner,
    ...
    /// </summary>
    public TursoEncryptionOptions Encryption
        => _encryption ?? throw new ObjectDisposedException(nameof(TursoEncryptionFileSystem));
    ...
    internal TursoEncryptionFileSystem WithInner(IFileSystem inner)
        => new(inner, Encryption, ownsEncryption: false);

    internal static IFileSystem Unwrap(IFileSystem fileSystem)
        => fileSystem is TursoEncryptionFileSystem encrypted ? encrypted._inner : fileSystem;

    ~TursoEncryptionFileSystem() => Dispose();
}
