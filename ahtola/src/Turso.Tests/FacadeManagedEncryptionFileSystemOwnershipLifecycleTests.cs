...
public sealed class FacadeManagedEncryptionFileSystemOwnershipLifecycleTests
{
    ...
    [Test]
    public void TursoConnectionDisposesItsEncryptionSnapshotAfterIdempotentClose()
    {
        var path = CreateDatabasePath("provider-close");
        try
        {
            using var connection = new global::Turso.TursoConnection(CreateConnectionString(path, Aes256Key));
            connection.Open();
    ...
    private static TursoEncryptionFileSystem GetOwnedEncryptionFileSystem(object connection)
    ...
    private static TursoEncryptionFileSystem? GetOwnedEncryptionFileSystemOrNull(object connection)
    {
        ...
        return (TursoEncryptionFileSystem?)field!.GetValue(connection);
    }
