...
public sealed class ManagedCoreParameterContractRegressionTests
{
    ...
    [Test]
    public void ManagedTursoFacadeBindsNumberedNamedAndPositionalValuesAfterRebind()
    {
        ...
        BindTursoParameters(command, 7L, "first", "positional");
        AssertTursoValues(command, 7L, "first", "positional");

        command.Parameters.Clear();
        Assert.Throws<InvalidOperationException>(() => command.ExecuteScalar())!
            .Message.Should().Be("Missing value for parameter ?1.");

        BindTursoParameters(command, 8L, "second", "rebound");
        AssertTursoValues(command, 8L, "second", "rebound");

        var outputParameter = new TursoParameter();
        Assert.Throws<ArgumentException>(() => outputParameter.Direction = ParameterDirection.Output);
    }
    ...
    private static void BindTursoParameters(TursoCommand command, long numbered, string named, string positional)
    {
        command.Parameters.Add(new TursoParameter("?1", numbered));
        command.Parameters.Add(new TursoParameter("$name", named));
        command.Parameters.Add(new TursoParameter(positional));
    }

    private static void AssertTursoValues(TursoCommand command, long numbered, string named, string positional)
    {
        using var reader = command.ExecuteReader();
        reader.Read().Should().BeTrue();
        reader.GetInt64(0).Should().Be(numbered);
