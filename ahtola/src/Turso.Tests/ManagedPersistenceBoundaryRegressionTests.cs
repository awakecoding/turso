...
public sealed class ManagedPersistenceBoundaryRegressionTests
{
    ...
    public void ManagedReadOnlyProvidersRetainTornWalWithoutWriteAccess()
    {
        ...
        try
        {
            using (var writer = new global::Turso.TursoConnection(
            ...
            {
                connection.Open();
                ExecuteTursoScalar(connection, "SELECT value FROM entries;").Should().Be(7);
            }
    ...
    public void ManagedEncryptionOptionsAreIndependentAcrossSuccessAndFailure()
    {
        ...
        try
        {
            using (var create = new global::Turso.TursoConnection(ManagedEncryptionConnectionString(path, Aes256Key)))
            {
                create.Open();
            ...
            {
                using var failedOpen = new global::Turso.TursoConnection(
                    ManagedEncryptionConnectionString(path, WrongAes256Key));
                failedOpen.Open();
            });

            using (var reopen = new global::Turso.TursoConnection(ManagedEncryptionConnectionString(path, Aes256Key)))
            {
                reopen.Open();
                ExecuteTursoScalar(reopen, "SELECT value FROM entries;").Should().Be(7);
            }

            using var options = TursoEncryptionOptions.FromHex(TursoEncryptionCipher.Aes256Gcm, Aes256Key);
            using var database = EmbeddedDatabase.OpenFile(
                copiedKeyPath,
                new TursoEncryptionFileSystem(PhysicalFileSystem.Instance, options));
            options.Dispose();
    ...
    private static long ExecuteTursoScalar(global::Turso.TursoConnection connection, string sql)
    {
