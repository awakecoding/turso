...
public class ManagedProviderCoreAdapterRouteTests
{
    ...
    {
        using var turso = new TursoConnection("Data Source=:memory:;Local Provider=Managed");
        turso.Open();

        GetPrivateField(turso, "_managedDatabase").Should().BeAssignableTo<IManagedDatabaseAdapter>();
        GetPrivateField(turso, "_nativeDatabase").Should().BeNull();

        using (var command = turso.CreateCommand())
        {
            command.CommandText = "SELECT $value;";
            command.Parameters.Add(new TursoParameter("$value", 42L));
        ...
        new TursoConnectionStringBuilder("Local Provider=Native").LocalProvider.Should().Be(TursoLocalProvider.Native);
        new SqliteConnectionStringBuilder("Local Provider=Native").LocalProvider.Should().Be(TursoLocalProvider.Native);
    }
