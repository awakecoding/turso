...
/// test time, so the claims behind <see cref="TursoDataAdapter"/> and <c>GetSchema</c> stay
...
public sealed class ManagedSchemaSqliteDifferentialTests
{
    ...
    public void ReservedWordsMatchesTheReferenceProviderExactly()
    {
        ...
        Turso().Should().BeEquivalentTo(Reference());

        static IEnumerable<string> Turso()
        {
            using var connection = OpenTurso(seed: false);
    ...
    {
        using var turso = OpenTurso(seed: false);
        using var reference = OpenReference(seed: false);

        Describe(turso.GetSchema("MetaDataCollections")).Should().Equal(Describe(reference.GetSchema("MetaDataCollections")));
        ...
        Collections(turso.GetSchema("MetaDataCollections"))
    ...
    public void ReaderTypeMetadataBeforeTheFirstReadMatchesTheReferenceProvider()
    {
        ...
        Probe(turso).Should().Equal(Probe(reference));
    ...
    public void ReaderSchemaTableMatchesTheReferenceProviderOnTheColumnsTheBuilderReads()
    {
        ...
        // IsKey is what TursoCommandBuilder uses to build the WHERE clause, and AllowDBNull and
    ...
    public void AdapterFillProducesTheReferenceColumnTypes()
    {
        ...
        Probe(turso, connection => new TursoDataAdapter(SelectSql, connection))
    ...
    private static TursoConnection OpenTurso(bool seed = true)
    {
