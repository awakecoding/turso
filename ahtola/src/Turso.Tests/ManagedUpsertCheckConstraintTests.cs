...
public sealed class ManagedUpsertCheckConstraintTests
{
    ...
    private static Turso.Data.Sqlite.SqliteConnection OpenManagedFile(string path)
    {
        var connection = new Turso.Data.Sqlite.SqliteConnection(
    ...
    private static IReadOnlyList<string> ReadFacade(Turso.Data.Sqlite.SqliteConnection connection, string sql)
    {
        var rows = new List<string>();
        using var command = connection.CreateCommand();
        command.CommandText = sql;
