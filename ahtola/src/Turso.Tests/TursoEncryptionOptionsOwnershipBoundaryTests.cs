...
[NonParallelizable]
public sealed class TursoEncryptionOptionsOwnershipBoundaryTests
{
    ...
    public void FileSystemOwnsEncryptionSnapshotAfterSuccessfulAndFailedOpens()
    {
        ...
        using var validOptions = TursoEncryptionOptions.FromHex(TursoEncryptionCipher.Aes256Gcm, Aes256Key);
        using var validFileSystem = new TursoEncryptionFileSystem(fileSystem, validOptions);
        using (var database = EmbeddedDatabase.OpenFile("ownership-success.db", validFileSystem))
        {
            validOptions.Dispose();
            Assert.Throws<ObjectDisposedException>(
                () => new TursoEncryptionFileSystem(fileSystem, validOptions));
        ...
        using var invalidOptions = TursoEncryptionOptions.FromHex(TursoEncryptionCipher.Aes256Gcm, WrongAes256Key);
        using var invalidFileSystem = new TursoEncryptionFileSystem(fileSystem, invalidOptions);
        ...
        Assert.Throws<ObjectDisposedException>(
            () => new TursoEncryptionFileSystem(fileSystem, invalidOptions));
