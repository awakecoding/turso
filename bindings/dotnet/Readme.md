...
- `PRAGMA page_size` accepts SQLite page sizes from 512 through 65536 as a pending value. `VACUUM`, `VACUUM main`, and attached-schema `VACUUM` rebuild the selected managed file; in-place page-size changes apply only in `DELETE` mode, while `VACUUM ... INTO <expression>` may apply the pending size in either journal mode without changing the source. `INTO` publishes a compact `DELETE`-mode snapshot through an atomic file-system replacement, retains the selected database's encryption and supported catalog/header semantics, and accepts only ordinary file paths on file systems that provide that guarantee. VACUUM rejects active transactions, result readers, blob handles, read-only/query-only connections, unsafe output aliases, and non-empty destinations before mutating authoritative storage.
