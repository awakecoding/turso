...
The managed `Turso` package exposes `System.Data.Common` types such as `DbConnection`, `DbCommand`, `DbDataReader`, `DbParameter`, `DbTransaction`, and `DbProviderFactory`. The `Turso.Raw` package contains the native interop layer and runtime assets used by the provider.
