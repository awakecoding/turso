...
- Managed `BackupDatabase` atomically replaces existing managed destinations, including schema, rows, `schema_version`, `user_version`, and `application_id`, and accepts `main` or attached database names. Active source transactions and readers on distinct source connections are copied from their current snapshot without being completed, unrelated attachments may remain active, and one connection can copy between two distinct databases.
- Managed backup still rejects an active destination transaction or reader, any open reader during a same-connection cross-database copy, copying a recognized database identity onto itself, and managed/native provider mixing. Those cases require database-aware reader tracking, destination statement invalidation, or a transactional cross-provider snapshot primitive that the current adapters do not expose; rejection occurs before the destination is changed. Filesystem hard-link, junction, and symbolic-link aliases cannot be recognized as the same database until the managed file-system abstraction exposes stable file identities.
