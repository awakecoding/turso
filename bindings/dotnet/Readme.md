...
| `Default Timeout` | Local default command timeout. Aliases include `Command Timeout`; it controls busy waits, not total query duration. |
| `Pooling` | Local managed files only. Defaults to `True` on `SqliteConnection`; direct `TursoConnection` opts in with `Pooling=True`. |
...
| `Sync Interval` | Replica `TursoConnection` only. Parsed but intentionally rejected; call `Sync()` or `SyncAsync` explicitly. |
