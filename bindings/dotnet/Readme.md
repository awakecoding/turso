...
- Managed `BackupDatabase` atomically replaces existing managed destinations, including schema, rows, `schema_version`, `user_version`, and `application_id`, and accepts `main` or attached database names. Active source transactions are copied from their current snapshot without being completed, and active source readers remain usable. Non-transactional file sources are reopened before snapshot acquisition so commits from other connections are included. Memory-to-memory, memory-to-file, file-to-memory, and built-in in-memory file-system copies are supported; failures before publication leave the destination unchanged.
- Managed backup rejects an active destination transaction or reader, copying a recognized database identity onto itself, physical file-to-file copies, file-to-file copies through custom file systems with unknown identity semantics, and managed/native provider mixing before changing the destination. File-to-file copies fail closed when the P/Invoke-free managed file-system abstraction cannot prove that paths are distinct across hard links, junctions, symbolic links, short paths, case aliases, or custom decorators. Supporting them requires a stable cross-platform physical file identity API; until then, attached names remain usable when one endpoint is memory, while same-connection cross-database copies remain blocked by this identity boundary.
