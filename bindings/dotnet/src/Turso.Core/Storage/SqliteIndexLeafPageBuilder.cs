...
/// <summary>
/// Packs a complete SQLite index-leaf page from records in SQLite BINARY order.
/// </summary>
/// <remarks>
...
public sealed class SqliteIndexLeafPageBuilder
{
    ...
    /// <summary>The appended cells in strict SQLite BINARY record order.</summary>
    ...
    public void Append(SqliteIndexLeafCell cell, ReadOnlySpan<byte> record)
    {
        ...
        if (_lastRecord is not null && RecordComparer.Compare(_lastRecord, record) >= 0)
            throw new ArgumentException("SQLite index records must be strictly increasing in BINARY order.", nameof(record));
