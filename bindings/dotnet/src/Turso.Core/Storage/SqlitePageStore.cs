...
public sealed class SqlitePageStore : IDisposable
{
    ...
    private SqlitePageStore(IFile file, SqliteDatabaseHeader header, TursoPageEncryption? encryption)
    {
        _file = file;
        _header = header;
        ...
        bool allowTrailingPages)
    {
        ...
        try
        {
            ...
            if (IsTursoEncrypted(rawHeader))
            {
                if (encryption is null)
                {
                    throw new InvalidDataException(
                        "Database is encrypted with Turso page encryption. Supply TursoEncryptionOptions; plaintext fallback is not permitted.");
                }
                ...
                if (header.ReservedSpace != TursoPageEncryption.MetadataSize)
                {
                    throw new InvalidDataException(
                        $"Encrypted Turso database reserves {header.ReservedSpace} bytes per page, but AES-GCM requires {TursoPageEncryption.MetadataSize}.");
                }
            }
            else
            {
    ...
    private static bool IsTursoEncrypted(ReadOnlySpan<byte> header)
        => header.Length >= 5 && header[..5].SequenceEqual("Turso"u8);
