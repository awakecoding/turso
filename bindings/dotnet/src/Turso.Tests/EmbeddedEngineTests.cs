...
public class EmbeddedEngineTests
{
    ...
    [Test]
    public void TriggerFiresOncePerRow()
    {
        var database = new EmbeddedDatabase();
        using var connection = database.Connect();
        Execute(connection, "CREATE TABLE t(id INTEGER);");
    ...
    [Test]
    public void BeforeTriggerRunsForEachInsertedRow()
    {
        var database = new EmbeddedDatabase();
        using var connection = database.Connect();
        Execute(connection, "CREATE TABLE t(id INTEGER);");
    ...
    public void TriggerWithOmittedTimingDefaultsToBefore()
    {
        ...
        Execute(
            connection,
            "CREATE TRIGGER trg INSERT ON t BEGIN INSERT INTO log VALUES (NEW.id); END;");
        Execute(connection, "INSERT INTO t VALUES (1);");
    ...
    [Test]
    public void ForEachRowClauseIsAccepted()
    {
        var database = new EmbeddedDatabase();
        using var connection = database.Connect();
        Execute(connection, "CREATE TABLE t(id INTEGER);");
        ...
        Execute(
            connection,
            "CREATE TRIGGER trg AFTER INSERT ON t FOR EACH ROW BEGIN INSERT INTO log VALUES (NEW.id); END;");
        Execute(connection, "INSERT INTO t VALUES (1), (2);");
    ...
    [Test]
    public void WhenClauseFiltersTriggerRows()
    {
        var database = new EmbeddedDatabase();
        using var connection = database.Connect();
        Execute(connection, "CREATE TABLE t(id INTEGER);");
    ...
    [Test]
    public void UpdateOfColumnsTriggerIsAccepted()
    {
        var database = new EmbeddedDatabase();
        using var connection = database.Connect();
        Execute(connection, "CREATE TABLE t(id INTEGER, name TEXT);");
        Execute(connection, "CREATE TABLE log(id INTEGER);");
        Execute(connection, "INSERT INTO t VALUES (1, 'before');");
        Execute(
            connection,
            "CREATE TRIGGER trg AFTER UPDATE OF name ON t BEGIN INSERT INTO log VALUES (1); END;");
        Execute(connection, "UPDATE t SET name = 'after';");

        AssertCount(connection, "SELECT COUNT(*) FROM log;", 1);
    }

    [Test]
    public void NewReferencesInTriggerBodyResolveAgainstTheInsertedRow()
    {
        var database = new EmbeddedDatabase();
        using var connection = database.Connect();
        Execute(connection, "CREATE TABLE t(id INTEGER);");
    ...
    [Test]
    public void SelectTriggerBodyStatementIsAccepted()
    {
        var database = new EmbeddedDatabase();
        using var connection = database.Connect();
        Execute(connection, "CREATE TABLE t(id INTEGER);");
