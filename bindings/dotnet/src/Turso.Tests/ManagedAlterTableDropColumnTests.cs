...
public sealed class ManagedAlterTableDropColumnTests
{
        ...
        bool encrypted)
    {
        ...
        using var encryption = encrypted
            ? TursoEncryptionOptions.FromHex(TursoEncryptionCipher.Aes256Gcm, Aes256Key)
            : null;
        IFileSystem fileSystem = encryption is null
            ? inner
            : new TursoEncryptionFileSystem(inner, encryption);
    ...
    private static string CreateDatabasePath()
        => Path.Combine(Path.GetTempPath(), $"turso-drop-column-{Guid.NewGuid():N}.db");
