...
using ManagedSqliteConnection = Turso.Data.Sqlite.SqliteConnection;
...
public sealed class ManagedAutoIncrementDurabilityTests
{
    ...
    public void EncryptedReopenPreservesSequence()
    {
        ...
        using (var encryption = TursoEncryptionOptions.FromHex(
                   ...
                   EncryptionKey))
        using (var fileSystem = new TursoEncryptionFileSystem(inner, encryption))
        ...
        using var reopenEncryption = TursoEncryptionOptions.FromHex(
            TursoEncryptionCipher.Aes256Gcm,
            EncryptionKey);
        using var reopenedFileSystem = new TursoEncryptionFileSystem(inner, reopenEncryption);
