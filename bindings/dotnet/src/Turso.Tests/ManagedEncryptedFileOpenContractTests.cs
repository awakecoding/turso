...
public class ManagedEncryptedFileOpenContractTests
{
    ...
    [Test]
    public void TursoManagedFileEncryptionCreatesReopensAndRejectsInvalidAccess()
    {
        var encryptedPath = CreateDatabasePath("provider");
        var plaintextPath = CreateDatabasePath("plaintext");
        try
        {
            using (var create = new global::Turso.TursoConnection(ManagedEncryptionConnectionString(encryptedPath, Aes256Key)))
            {
                create.Open();
            ...
            File.ReadAllBytes(encryptedPath).AsSpan(0, 5).ToArray().Should().Equal("Turso"u8.ToArray());

            using (var reopen = new global::Turso.TursoConnection(ManagedEncryptionConnectionString(encryptedPath, Aes256Key)))
            {
                reopen.Open();
            ...
            {
                using var missingKey = new global::Turso.TursoConnection(
            ...
            Assert.Throws<InvalidDataException>(() =>
            {
                using var wrongKey = new global::Turso.TursoConnection(
            ...
            Assert.Throws<InvalidDataException>(() =>
            {
                using var tampered = new global::Turso.TursoConnection(
                    ManagedEncryptionConnectionString(encryptedPath, Aes256Key));
                tampered.Open();
            })!.Message.Should().Contain("failed authentication");

            using (var plaintext = new global::Turso.TursoConnection(
            ...
            Assert.Throws<InvalidDataException>(() =>
            {
                using var encryptedOpen = new global::Turso.TursoConnection(
    ...
    public void SqliteFacadeManagedFileEncryptionUsesExplicitCipherAndKeyOptions()
    {
        ...
        try
        {
            ...
            {
                DataSource = path,
                LocalProvider = TursoLocalProvider.Managed,
    ...
    public void EncryptedPhysicalPagerFactoryEncryptsWalFramesAndReopensThem()
    {
        ...
        try
        {
            ...
            var fileSystem = new TursoEncryptionFileSystem(PhysicalFileSystem.Instance, encryption);
