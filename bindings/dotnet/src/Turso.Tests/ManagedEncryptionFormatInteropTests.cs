...
public sealed class ManagedEncryptionFormatInteropTests
{
    ...
    public void ManagedWriterUsesTursoFormatVersionZeroAndExactAesCipherIds(
        StorageCipher cipher,
        string key,
        byte cipherId)
    {
        var fileSystem = new InMemoryFileSystem();
        using var encryption = TursoEncryptionOptions.FromHex(cipher, key);
        using (SqlitePageStore.Create(fileSystem, "format.db", encryption: encryption))
        {
        }

        var firstPage = ReadFile(fileSystem, "format.db");
        firstPage.AsSpan(0, 5).ToArray().Should().Equal("Turso"u8.ToArray());
    ...
    public void ManagedReaderRejectsUnsupportedFormatVersionWithoutFallback()
    {
        ...
        using (encryption)
        {
            ...
            Assert.Throws<InvalidDataException>(
                ...
                .Message.Should().Be(
                    "Unsupported Turso encrypted database format version 1; managed storage supports only "
    ...
    public void ManagedReaderRejectsEveryNonAesCipherIdWithoutInference(byte cipherId, string cipherName)
    {
        ...
        using (encryption)
        {
            ...
            Assert.Throws<InvalidDataException>(
                () => SqlitePageStore.Open(fileSystem, "format.db", encryption: encryption))!
                .Message.Should().Be(
                    $"Encrypted database uses Turso cipher ID {cipherId} ({cipherName}); managed storage supports "
    ...
    {
        var fileSystem = new InMemoryFileSystem();
        using var aes256 = TursoEncryptionOptions.FromHex(StorageCipher.Aes256Gcm, Aes256Key);
        using (SqlitePageStore.Create(fileSystem, "format.db", encryption: aes256))
        {
        }

        using var aes128 = TursoEncryptionOptions.FromHex(StorageCipher.Aes128Gcm, Aes128Key);
        Assert.Throws<InvalidDataException>(
            () => SqlitePageStore.Open(fileSystem, "format.db", encryption: aes128))!
            .Message.Should().Be(
                "Encrypted database uses Turso cipher ID 2 (AES-256-GCM), but the supplied options specify "
        ...
        int requiredKeyLength)
    {
        Assert.Throws<ArgumentException>(
            () => TursoEncryptionOptions.FromHex(cipher, key))!
    ...
    {
        Assert.Throws<ArgumentException>(
            () => TursoEncryptionOptions.FromHex(StorageCipher.Aes256Gcm, "not-hex"))!
    ...
    public void ManagedReaderRejectsAuthenticatedHeaderReservedByteTampering()
    {
        ...
        using (encryption)
        {
            ...
            Assert.Throws<InvalidDataException>(
                () => SqlitePageStore.Open(fileSystem, "format.db", encryption: encryption))!
                .Message.Should().Be("Turso encrypted database header has non-zero reserved bytes.");
        }
    }
    ...
    {
        var fileSystem = new InMemoryFileSystem();
        using var encryption = TursoEncryptionOptions.FromHex(StorageCipher.Aes256Gcm, Aes256Key);
    ...
    [TestCase("AEGIS128X2")]
    [TestCase("AEGIS128X4")]
    public void TursoManagedConnectionRejectsUnimplementedCipherNamesAtConfigurationBoundary(string cipher)
    {
        using var connection = new global::Turso.TursoConnection(
            $"Data Source=test.db;Local Provider=Managed;Encryption Cipher={cipher};Encryption Key={Aes256Key}");
    ...
    private static (InMemoryFileSystem FileSystem, TursoEncryptionOptions Encryption) CreateEncryptedStore()
    {
        var fileSystem = new InMemoryFileSystem();
        var encryption = TursoEncryptionOptions.FromHex(StorageCipher.Aes256Gcm, Aes256Key);
        using (SqlitePageStore.Create(fileSystem, "format.db", encryption: encryption))
        {
        }
    ...
    private static SqlitePager CreatePager(InMemoryFileSystem fileSystem, TursoEncryptionOptions encryption)
        => SqlitePager.Create(
            fileSystem,
            "format.db",
            "format.db-wal",
