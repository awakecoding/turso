...
public class ManagedMigrationsLockConcurrencyTests
{
    ...
    {
        var path = Path.Combine(Path.GetTempPath(), $"turso-defect-b-{Guid.NewGuid():N}.db");
    ...
    public void ConcurrentMigratorsOneShotRotateTheLockAtNativeCadence()
    {
        ...
        var path = Path.Combine(Path.GetTempPath(), $"turso-miglock-oneshot-{Guid.NewGuid():N}.db");
        var logPath = Path.Combine(Path.GetTempPath(), $"turso-miglock-oneshot-{Guid.NewGuid():N}.log");
    ...
    public async Task ConcurrentMigratorsOneShotAsyncRotateTheLock()
    {
        ...
        var path = Path.Combine(Path.GetTempPath(), $"turso-miglock-async-{Guid.NewGuid():N}.db");
        var logPath = Path.Combine(Path.GetTempPath(), $"turso-miglock-async-{Guid.NewGuid():N}.log");
        var log = new ConcurrentQueue<string>();
        var connections = new List<SqliteConnection>();
        try
        {
    ...
    public async Task ConcurrentMigratorsEfMigrateAsyncShapeRotateTheLock()
    {
        var workers = Environment.ProcessorCount;
        var budget = TimeSpan.FromSeconds(240);
        var path = Path.Combine(Path.GetTempPath(), $"turso-miglock-efasync-{Guid.NewGuid():N}.db");
        var logPath = Path.Combine(Path.GetTempPath(), $"turso-miglock-efasync-{Guid.NewGuid():N}.log");
        var log = new ConcurrentQueue<string>();
        var connections = new List<SqliteConnection>();
        try
        {
    ...
    private void RunLockConvoy(int workers, TimeSpan budget, bool useEfBackoff)
    {
        var path = Path.Combine(
            Path.GetTempPath(),
            $"turso-miglock-{Guid.NewGuid():N}.db");
        var connections = new List<SqliteConnection>();
        try
        {
            using (var seed = new SqliteConnection($"Data Source={path};Local Provider=Managed"))
