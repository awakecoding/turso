...
public sealed class SeekParityTests
{
    ...
    // P1-21 tie-ordering probes. ORDER BY over a NON-unique column with out-of-order explicit
    // rowids + ties isolates the tie-break order. real SQLite (and MDS) scan rows in rowid order
    // and sort stably, so ties resolve in ROWID order. The managed engine scans table.Rows in
    // INSERT order (CommitInserts appends; only WITHOUT-ROWID tables re-sort) and CompareRows has
    // no rowid tiebreak, so the sorter would resolve ties in INSERTION order — a divergence for
    // out-of-order rowids. These probes settle which behavior the managed engine has today.
    [Test]
    public void OrderByNonUniqueColumnWithOutOfOrderRowidsMatchesSqlite()
    {
        // g='a' ties rowid 10 ('ten') and rowid 3 ('three'). MDS rowid-order ties -> three,ten.
        AssertSelectMatchesSqlite(
    ...
    {
        // DESC: g='b' ('seven') first, then g='a' ties rowid 3,10 -> three,ten.
        AssertSelectMatchesSqlite(
